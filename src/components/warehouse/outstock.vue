      ...
      <Layout>
        ...
        <Layout :style="{padding: '0 24px 24px'}">
          <Breadcrumb :style="{margin: '24px 0'}">
            ...
            <BreadcrumbItem>出库登记</BreadcrumbItem>
          </Breadcrumb>
          <Content :style="{padding: '24px', minHeight: '280px', background: '#fff'}">
            <Table border :columns="columns12" :data="listClassify" style="margin-top:20px">
              ...
              <template slot-scope="{ row, index }" slot="action">
                <Button type="success" size="small" @click="stock(index)">可出库</Button>
              </template>
            </Table>
            <template>
...
export default {
  ...
  mounted() {
    Axios.get("/api/main/sell/somain/show?type=" + 2).then(resp => {
  ...
  methods: {
    ...
    stock(index){
        Axios.post('/api/main/stock/outstock?'+Qs.stringify({soId:this.listClassify[index].soId,payType:this.listClassify[index].payType})).then(resp => {
            console.log(resp);
        })
    }
  }
