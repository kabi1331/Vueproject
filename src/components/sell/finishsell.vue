      ...
      <Layout>
        ...
        <Layout :style="{padding: '0 24px 24px'}">
          <Breadcrumb :style="{margin: '24px 0'}">
            ...
            <BreadcrumbItem>销售单了结</BreadcrumbItem>
          </Breadcrumb>
...
export default {
  ...
  watch: {
    model1() {
      Axios.get("/api/main/sell/somain/show?" + Qs.stringify({type:4,payType:this.model1})).then(
        resp => {
  ...
  methods: {
    pIndexChange(i) {
      this.i = i;
      Axios.get("/api/main/sell/somain/query?" + {page:this.i,payType:this.model1}).then(resp => {
        this.listClassify = resp.data.list;
      });
    },
    put(i) {
      Axios.post("/api/main/sell/somain/end?" + Qs.stringify({soId:this.listClassify[i].soId,payType:this.listClassify[i].payType})).then(resp => {
        console.log(resp);
      })
    }
  }
