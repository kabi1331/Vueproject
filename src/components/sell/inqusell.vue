      ...
      <Layout>
        ...
        <Layout :style="{padding: '0 24px 24px'}">
          <Breadcrumb :style="{margin: '24px 0'}">
            ...
            <BreadcrumbItem>销售单查询</BreadcrumbItem>
          </Breadcrumb>
          <Content :style="{padding: '24px', minHeight: '280px', background: '#fff'}">
            <Row>
              采购单编号
              <Input placeholder="Enter something..." style="width:100px" v-model="soId" />日期范围
              <DatePicker
                type="daterange"
...
export default {
  ...
  mounted() {
    Axios.get("/api/main/sell/somain/query").then(resp => {
      this.listClassify = resp.data.list;
    });
  },
  watch: {
    model1() {
      Axios.get("/api/main/sell/somain/query?status=" + this.model1).then(
        resp => {
          this.listClassify = resp.data.list;
  ...
  methods: {
    ...
    pIndexChange(i) {
      this.i = i;
      Axios.get("/api/main/sell/somain/query ?page=" + this.i).then(resp => {
        this.listClassify = resp.data.list;
      });
    }
  }
