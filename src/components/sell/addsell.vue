      ...
      <Layout>
        ...
        <Layout :style="{padding: '0 24px 24px'}">
          <Breadcrumb :style="{margin: '24px 0'}">
            ...
            <BreadcrumbItem>新添销售单</BreadcrumbItem>
          </Breadcrumb>
...
export default {
  ...
  watch: {
    model1() {
      Axios.get("/api/main/sell/somain/show?type=" + this.model1).then(
        resp => {
  ...
  methods: {
    changetest(index, row) {
      this.$router.push("/changesell/"+this.listClassify[index].soId);
    },
    remove(index) {
      Axios.post(
        "/api/main/sell/somain/delete?soId=" +
          this.listClassify[index].soId
      ).then(resp => {
        this.listClassify.splice(index, 1);
      });
    },
    gotoadd() {
      this.$router.push("/newsell");
    },
    pIndexChange(i) {
      this.i = i;
