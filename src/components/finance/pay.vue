      ...
      <Layout>
        ...
        <Layout :style="{padding: '0 24px 24px'}">
          <Breadcrumb :style="{margin: '24px 0'}">
            ...
            <BreadcrumbItem>付款登记</BreadcrumbItem>
          </Breadcrumb>
            ...
            <Table border :columns="columns12" :data="listClassify" style="margin-top:20px">
              ...
              <template slot-scope="{ row, index }" slot="action">
                <Button type="success" size="small" @click="put(index)">
                    {{row.payType==3&&row.status==1?'付预付款':'付款'}}
                </Button>
              </template>
            </Table>
            <template>
...
export default {
  ...
  mounted() {
    Axios.get("/api/main/purchase/pomain/show?type=" + 3).then(resp => {
  ...
  methods: {
    ...
    put(index){
        ...
        Axios.post('/api/main/finance/pay?'+Qs.stringify({poId:this.listClassify[index].poId,type:this.type})).then(resp => {
            console.log(resp);
        })
    }
  }
